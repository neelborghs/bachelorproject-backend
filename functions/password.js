	...
	new Promise((resolve, reject) => {
		...
		.then(user => {
			...
			const mailOptions = {
    			...
    			Your password reset token is <b>${random}</b><br>
    			This token is valid for only 2 minutes, so please change your password right away!<br><br>
    			Thanks,<br>
    			EZgreen Team.`
		...
		.then(info => {
			console.log(info);
			resolve({ status: 200, message: 'Check your mail for instructions' })
		})
